...
export function quoteLiteral(value: unknown): string {
  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i += 1) {
      if (Array.isArray(value[i])) {
        temp.push(arrayToList(i !== 0, value[i] as unknown[], quoteLiteral));
      } else {
  ...
  if (Array.isArray(value)) {
    ...
    for (let i = 0; i < value.length; i += 1) {
      if (value[i] !== null && value[i] !== undefined) {
        if (Array.isArray(value[i])) {
          temp.push(arrayToList(i !== 0, value[i] as unknown[], quoteString));
        } else {
...
export function formatWithArray(fmt: string, parameters: FormatValue[]): string {
  let index = 0;
