...
export class AggregateFunctionBuilder extends RangeFunctionBuilder {
    ...
    override filter(condition?: Statement) {
    ...
    // orderByAgg(...cols: Statement[]) {
    //     this.query.sql.push("ORDER", "BY");
    //     const normalized = super.normalizeStatementArray(cols);
    //     const resolvedColumns = normalized.map((item) => super.resolveIdentifierStatement(item));
    //     super.pushSeparatedTokens(resolvedColumns, ",");
    //     return this;
    // }

    withinGroup(orderByCol?: Statement) {
        this.query.sql.push("WITHIN", "GROUP", "(");
        if (orderByCol) {
            this.query.sql.push("ORDER", "BY");
            const resolved = super.resolveIdentifierStatement(orderByCol);
