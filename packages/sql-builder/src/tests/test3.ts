...
// Parse SQL to AST
// let stmts = await parse(`SELECT  service , sub_service , sla,  count(1) as jumlah_keluhan,
...
//                         ORDER BY 1`);
let stmts = await  parse("BEGIN; SELECT admins.id FROM admins UNION SELECT moderators.id FROM moderators; COMMIT;")
let stmts2 = await  parse("SELECT CASE WHEN TRUE THEN 1 ELSE 2 END  something FROM users")

// stmt = 

// console.log(JSON.stringify(stmts, null, 2));
// Using a simple walker function
const uniqueKey = new Set<string | number>();
const walker: Walker = (path: NodePath) => {
    uniqueKey.add(path.key);
...
// walk(stmts, walker);
console.log(
    JSON.stringify(await stmts, null, 2),
    // JSON.stringify(await stmts2, null, 2),
    // JSON.stringify(await stmts, null, 2)  == JSON.stringify(await stmts2, null, 2),
);

// check if object has same key length iteratively
...
//         if ((areObjects && !haveSameKeys(val1, val2))) {
//             console.log(keys1, keys2);
//             return false;
...
// console.log(haveSameKeys(stmts, stmts2));