...
describe("Text Search Functions", () => {
    ...
    describe("Function Overload Resolution", () => {
        ...
        describe("tsRank overload resolution", () => {
            test("should accept two arguments (vector, query)", () => {
                const query = qb.tsRank(qb.i("search_vector"), qb.toTsquery(qb.l("cat"))).getSql();
                expect(query).toContain("TS_RANK");
            });

            test("should accept three arguments (vector, query, normalization)", () => {
                const query = qb.tsRank(qb.i("search_vector"), qb.toTsquery(qb.l("cat")), qb.l(2)).getSql();
                expect(query).toContain("TS_RANK");
            });

            test("should accept four arguments (weights, vector, query, normalization)", () => {
        ...
        describe("tsHeadline overload resolution", () => {
            test("should accept two arguments (document, query)", () => {
                const query = qb.tsHeadline(qb.l("The fat cat"), qb.toTsquery(qb.l("cat"))).getSql();
                expect(query).toContain("TS_HEADLINE");
            });

            test("should accept three arguments (config, document, query)", () => {
