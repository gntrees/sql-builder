import type { Node, ParseResult } from "../../src/ast-types";
...
interface Checkpoint {
    ...
    child: CheckpointType;
}

type CheckpointType = Node | string | null;
 ...
 */
export function normalizeLocationsNode(parseResult: ParseResult, sql: string): ParseResult {
 ...
 */
function collectAllLocations(parseResult: ParseResult): Set<number> {
 ...
 */
function findNextCheckpoint(position: number, checkpoints: Set<number>, fallback: number): number {
 ...
 */
function processNode(node: any, parentLocationStart: number, parentEnd: number, sql: string, checkpoints: Set<number>): any {
    if (!node || typeof node !== "object") {
        return node;
    }
    ...
    const result: NodeWithLocations = {
        ...processedChildren,
 ...
 */
function getNodeStart(node: any, fallback: number): number {
    if (!node || typeof node !== "object") {
        return fallback;
    }
 ...
 */
function calculateNodeEnd(node: any, nodeValue: any, nodeStart: number, parentEnd: number, checkpoints: Set<number>): number {
    const nodeKey = Object.keys(node)[0] ?? "";
 ...
 */
function processChildren(nodeValue: any, parentLocationStart: number, parentEnd: number, sql: string, checkpoints: Set<number>): any {
    const result: any = {};
