...
const specialNode = {
        ...
        if (currentNode.SelectStmt.fromClause && currentNode.SelectStmt.fromClause.length > 0) {
            for (const fromItem of currentNode.SelectStmt.fromClause) {
                ...
                if (fromKey === 'RangeVar') {
                    ...
                    const fromExpr = fromItem[fromKey];
    ...
    RangeVar: (node) => {
        const currentNode = normalizeNode(node, 'RangeVar');
    ...
    JoinExpr: (node) => {
        const currentNode = normalizeNode(node, 'JoinExpr');
        ...
        if (tableKey === 'RangeVar') {
            const rangeVar = tableNode.RangeVar;
    ...
    BoolExpr: (node) => {
        const currentNode = normalizeNode(node, 'BoolExpr');
    ...
    SortBy: (node) => {
        const currentNode = normalizeNode(node, 'SortBy');
