...
export const resolveNode = (node:{
    ...
    const resolver = specialNode[tag as keyof typeof specialNode] ?? null;
    if (resolver) {
...
export const fallbackNode = (node: any): FunctionListType[] => {
    const raw = resolveRaw(node);
    console.log(`No resolver found for ${Object.keys(node)[0]}, raw : ${deparseSync(node)}`);
    return raw
}

export const normalizeNode = <T,K extends string>(key: K,node:any): {[P in K]:T} => {
    const tag = Object.keys(node)[0];
...
export const resolveNodeArray = (
    nodes: any[],
): FunctionListType[] => {
    const params: FunctionListType[] = [];
...
export const escapeForTemplateLiteral = (str: string): string => {
