...
const specialNodeTransactions: Record<string, (node: any) => FunctionListType[]> = {
    TransactionStmt: (rawNode: any): FunctionListType[] => {
        const results: FunctionListType[] = [];
        const node = normalizeNode<TransactionStmt, "TransactionStmt">("TransactionStmt", rawNode);
        const tempFunction: FunctionListType = {
            name: '',
            arguments: [],
            paramType: 'function'
        }
        const kindMap: Record<TransactionStmtKind, string> = {
